...
int read_value; // variable for reading the gas status
int set = 350;  // we start, assuming Gas detected
...
void loop(){
  ...
  if(read_value>set){ // check if the Gas level is High
    lcd.setCursor(0, 1);
    lcd.print("Alert....!!!    ");    
  ...
  if(read_value<set){ // check if the Gas level is Low
    lcd.setCursor(0, 1);
    lcd.print(".....Normal.....");   
